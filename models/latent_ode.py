#CODE FROM: https://github.com/rtqichen/torchdiffeq/blob/master/examples/latent_ode.py
...
def generate_spiral2d(nspiral=1000,
    ...
    for _ in range(nspiral):
        ...
        t0_idxs.append(t0_idx)

        #Adding relevant times to the list
        samp_ts_local = orig_ts[t0_idx:t0_idx + nsample]
        samp_ts_list.append(samp_ts_local)

    # batching for sample trajectories is good for RNN; batching for original
    # trajectories only for ease of indexing
    orig_trajs = np.stack(orig_trajs, axis=0)
    samp_trajs = np.stack(samp_trajs, axis=0)
    samp_ts = np.stack(samp_ts_list, axis=0)
...
class RecognitionRNN(nn.Module):
    ...
    def initHidden(self, nbatch=None):
        n = nbatch if nbatch is not None else self.nbatch
        return torch.zeros(n, self.nhidden)
