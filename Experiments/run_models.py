...
def plot_spiral_extrapolation(test_dict, model, epoch, experimentID, save_dir="ODE-RNN_Results/spiral_plots", plot_indices=None):
	...
	with torch.no_grad():
		pred_y, _ = model.get_reconstruction(
			test_dict["tp_to_predict"], 
			test_dict["observed_data"], 
			test_dict["observed_tp"], 
	...
	fig.suptitle(f"Spiral Extrapolation, Epoch {epoch:04d}", y=0.98)
	plt.tight_layout(rect=[0, 0.05, 1, 0.95])

	save_path = os.path.join(save_dir, f"spiral_extrap_exp{experimentID}_epoch{epoch:04d}.png")
	...
	for itr in range(1, num_batches * (args.niters + 1)):
		...
		if itr % (n_iters_to_viz * num_batches) == 0:
			with torch.no_grad():
				...
				if "pois_likelihood" in test_res and args.poisson:
					logger.info("Poisson likelihood: {}".format(test_res["pois_likelihood"]))

				if "ce_loss" in test_res and args.classif:
