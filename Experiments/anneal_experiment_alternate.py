...
def visualize(func, rec, dec, orig_trajs, samp_trajs, samp_ts, orig_ts_np, latent_dim, device, save_path):
    with torch.no_grad():
        ...
        for plot_i, spiral_idx in enumerate(VIZ_SPIRAL_IDX):
            ...
            h = rec.initHidden(nbatch=nspiral).to(device)
...
if __name__ == '__main__':
    ...
    log_path = f'./log-{args.niters}-{args.lr}-{anneal_label}-no_time_share.csv'
    ...
    if args.visualize:
        fig_name = f'./vis-{args.niters}-{args.lr}-{anneal_label}-no_time_share.png'
        visualize(
            func, rec, dec,
